...
public class GridViz : MonoBehaviour {
    ...
    private void Start() {
        grid = new Grid(130, 130, 1f, new Vector3(45, 45));

        heatmap = new HeatMapVisual(grid, GetComponent<MeshFilter>());
    }
    ...
    private void HandleHeatMapMouseMove() {
        ...
        if (mouseMoveTimer < 0f) {
            ...
            float gridValue = grid.GetValue(UtilsClass.GetMouseWorldPosition());
